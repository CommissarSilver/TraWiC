...
def get_model_output(file_path):
    ...
    with open(
        os.path.join(
            args.working_dir,
            "run_results",
            f"TokensRun_mistral_{args.run_num}_2",
            "results.jsonl",
        ),
        "a",
    ) as f:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(
        os.path.join(
            args.working_dir, "run_results", f"TokensRun_mistral_{args.run_num}_2"
        )
    ):
        os.mkdir(
            os.path.join(
    ...
    try:
        files_generated_blocks = open(
            os.path.join(args.working_dir, "run_results", "generated_mistral.txt"), "r"
