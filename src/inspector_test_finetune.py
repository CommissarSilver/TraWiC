...
# final results file
results_file = open(
    f"clf_{args.model_name}_{args.model_epoch}_{args.classifier}.csv",
    "a",
)
results_file.write(
...
with open(
    f"inspector_test_file_level_clf_{args.classifier}_{args.model_name}_{args.model_epoch}__syn{args.syntactic_threshold}_sem{args.semantic_threshold}.csv",
    "w",
) as f:
    ...
    for i in tqdm(range(len(x))):
        ...
        repo_name = combined_ds.iloc[i, 0].split("/")[1]
...
with open(
    f"inspector_test_repo_level_0.4_clf_{args.classifier}_{args.model_name}_{args.model_epoch}__syn{args.syntactic_threshold}_sem{args.semantic_threshold}.csv",
    "w",
) as f:
...
with open(
    f"inspector_test_repo_level_0.6_clf_{args.classifier}_{args.model_name}_{args.model_epoch}__syn{args.syntactic_threshold}_sem{args.semantic_threshold}.csv",
    "w",
) as f:
    f.write("repo_name,predicted,actual\n")
    for k, v in final_repo_result.items():
