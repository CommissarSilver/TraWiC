...
model = Roberta() if args.model == "roberta" else None
...
def get_model_output(file_path):
    ...
    # model_inputs = [
    #     input["prefix"] + " <mask> " + input["suffix"]
    ...
    for candidate_input in tqdm(model_inputs):
        model_output = model.infill(
            candidate_input["prefix"] + " <mask> " + candidate_input["suffix"]
        )
        if model_output == "too_many_tokens":
            f = open(
                os.path.join(os.getcwd(), "run_results", "too_many_tokens.txt"), "a"
            )
            f.write(file_path + "\n")
            return None
        else:
    ...
    with open(
        os.path.join(
            os.getcwd(),
            "run_results",
            f"RobertaTokensRun{args.run_num}",
            "results.jsonl",
        ),
        "a",
    ) as f:
